...
def evaluate_model(model, x_test, y_test, mapping_time, mapping_difficult):
    ...
    report_time = classification_report(
        y_test.iloc[:, 0], y_pred[:, 0], output_dict=True
    )
    report_difficult = classification_report(
        y_test.iloc[:, 1], y_pred[:, 1], output_dict=True
    )
    ...
    df_difficult[["precision", "recall", "f1-score"]].plot(
        kind="bar", rot=0, color=["#1f77b4", "#ff7f0e", "#2ca02c"]
    )
    ...
    plt.bar(
        ["Time", "Difficult"],
        [accuracy_time, accuracy_difficult],
        color=["#9467bd", "#8c564b"],
    )
